# WTFPL Frequently Asked Questions (ja_JP)

これは [WTFPL – Do What the Fuck You Want to Public License のFAQページ](https://www.wtfpl.net/faq/) の内容の一部を日本語に意訳したものである。

* このコンテンツは 2026-09-11 時点でのWebサイトの記述内容に基づく
* このコンテンツは完全ではないし、正しい翻訳とも限らない

## WTFPLを利用するにはどうすればよいでしょうか?

**ステップ1** WTFPL の [全文](https://www.wtfpl.net/txt/copying) をダウンロードまたはコピー&ペーストし、あなたの著作物に同梱します。

ライセンスファイルの一般的なファイル名は `COPYING` です。
もし複数のライセンスを採用しているなら、WTFPLのライセンスファイル名は `COPYING.WTFPL` とするのが妥当でしょう。

**ステップ2** 著作物の著作権表示に、以下の文言を追加します。

```
Copyright © 2000 Your Name <your@address>
This work is free. You can redistribute it and/or modify it under the
terms of the Do What The Fuck You Want To Public License, Version 2,
as published by Sam Hocevar. See the COPYING file for more details.
```

もし現実的な問題として著作物に `COPYING` ファイルを同梱できないのであれば、上記のライセンス文を著作物に直接コピー&ペーストしてください。
もしくは、以下のようにこのサイトへリンクすることもできます。

```
Copyright © 2000 Your Name <your@address>
This work is free. You can redistribute it and/or modify it under the
terms of the Do What The Fuck You Want To Public License, Version 2,
as published by Sam Hocevar. See http://www.wtfpl.net/ for more details.
```

## WTFPLの各条項を説明してください

WTFPLは3つの部分から成り立っています:

(image)

ライセンスの読み方は以下のとおりです

**(1)** これはライセンスの名前とバージョンを示します

**(2)** これはライセンス自体に関するルールです。 Sam Hocevar がこのライセンスを作成しました。ライセンスを改変することも可能ですが、改変したものをWTFPLと呼ぶことはできない、と書いてあります。

**(3)** これは著作物に関するルールです。 単に **DO WHAT THE FUCK YOU WANT TO** (お前の好きにせえよクソが) とだけ書いてあります。

## ソフトウェア名を変更することができないの? そんなものフリーソフトウェア・ライセンスとは言えないのでは?

WTFPLではソフトウェア名を変えることも **許されます** 。前項をもう一度よく読んでください。

## WTFPLは有効な（本当に通用する）ライセンスですか?

WTFPLの有効性が裁判所で検証されたことはありませんが、有効なライセンスとして広く受け入れられています。

主なLinuxディストリビューション (Debian, Fedora, Arch, Gentoo, etc) は、 WTFPL version 1 もしくは 2 で
ライセンスされたソフトウェアを同梱しています。
[この引用](http://lists.debian.org/debian-legal/2002/09/msg00032.html) によれば、Bradley Kuhn フリーソフトウェア財団事務局長は「FSFの人々は、確かにWTFPLは有効なフリーソフトウェア・ライセンスだと認めた」と話しています。

## なぜ無保証条項がないのですか?

WTFPL はコンピュータ・プログラムに限らず、アートワークや文章などにも利用できる汎用ライセンスです。
そのため WTFPL はコピー・頒布・改変のみをカバーしています。

もしプログラムに無保証条項を追加したい場合は、以下のような表現でソースコード中に示すことができます。

```
/* This program is free software. It comes without any warranty, to
 * the extent permitted by applicable law. You can redistribute it
 * and/or modify it under the terms of the Do What The Fuck You Want
 * To Public License, Version 2, as published by Sam Hocevar. See
 * http://www.wtfpl.net/ for more details. */
```

## このライセンスは本質的にパブリック・ドメインと同じではありませんか?

残念ながらパブリック・ドメインの定義は管轄区域によって異なります。
死後70年を経過していない場合、著作者が自らの著作物をパブリック・ドメインにできるかどうか自体、地域によって議論の余地があります。

## ライセンスの汚い言葉遣いを直せませんか? 不適切で、幼稚で、企業に相応しくない

WTFPLの下において、あなたは著作物を他のライセンスで再ライセンスすることができます。

## 下品だって言ってんのが分からないか?

お前の「下品」に対するスタンスなど知ったことではない。お前の人生の価値観なんて知ったことじゃない。

WTFPLとは *fucking freedom* ——『自由』を示すライセンスだ。本気でそう思って名付けたんだ。

自由という言葉は、何も芸術や科学的成果を人類で分かち合ったり、コピーしたり、改変するような行為だけを示すものじゃない。
同性愛者である自由、結婚して滅茶苦茶なゲイセックスをする自由、体に刺青を彫り込む自由、神など存在しないと宣言する自由、避妊薬を飲む自由、中絶を行う自由、それらすべてが「自由」という言葉に含まれている。
**同性愛者や無神論者であったがために人は死ぬ。**
"Fuck" から守るべき崇高な何かがあるなどと言ってくれるな。もしお前が WTFPL の `Fuck` が不快だの下品だのとのたまうなら、お前も、そんなお前の信念も、すべてFuckだ。

Triple Fuck You.

## 実際にWTFPLを使った著作物はありますか?

このサイト上の WTFPL ライセンスは version 2 ですが、WTFPL version 1 は Banlu Kemiyatorn によって作成され、
彼がいくつかの [WindowMaker のアートワーク](https://github.com/window-maker/wmaker/blob/master/COPYING.WTFPL) に使用していました。

また、このサイトの [showcase](https://www.wtfpl.net/showcase/) からWTFPLを利用した著作物を見ることができます。

## WTFPLソフトウェアを収益目的で利用してもよいですか?

はい。

## ところで、WTFPLの作品でこんなことをしたいたんだけど……

もちろんかまいません。

## まさかこんなことでも……

かまいません。

## あんなことでも?

かまわん。

## フロリアンに何か好きなものなんてあるだろうか?

それはない。
